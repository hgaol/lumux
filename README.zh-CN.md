...
一个常驻的左侧边栏（herdr 风格）：上半部分列出你的**会话**，下半部分列出每个正在运行 **agent** 的窗格及其实时状态——空闲（`○`）、工作中（动画转轮）、被阻塞/等待输入（红色 `●`）、已完成（`✓`）。点击会话行即可切换过去，点击 agent 行则直接跳到该 agent 的具体窗格。prefix-`s` 选择器的每个窗口行也显示相同状态。

agent **在其进程启动的那一刻就会出现**——lumux 会检测每个窗格中运行的 agent，因此无需等待任何 hook，进程退出时也会自动消失。

点击 SESSIONS 表头上的 **`+`** 可新建会话，点击 **`◀`** 可把侧边栏收起为细条（**`▶`** 再展开）。

**右键**点击会话行、状态栏中的窗口或某个窗格会弹出上下文菜单——会话可重命名/新建窗口/关闭，窗口可重命名/关闭，窗格可分割/缩放/关闭。按 `Esc` 或点击别处即可关闭菜单。

同一个菜单也可以用键盘打开：**`prefix M`**（当前窗格），或 `:menu window` / `:menu session`。方向键移动选中项，`Enter` 执行，`Esc` 关闭。这一点值得留意，因为有些终端会把右键留给自己，根本不会转发给应用程序：

| 终端 | 右键 |
| --- | --- |
| Windows Terminal 及多数 Unix 终端 | 转发给 lumux——除非按住 **Shift**，那是通向终端自身菜单的预留出口 |
| VS Code 内置终端 | 各平台默认都不转发（Windows 上是复制/粘贴，其他平台是它自己的右键菜单）；设置 `"terminal.integrated.rightClickBehavior": "nothing"` 才会放行 |

`lumux debug-input` 会打印终端实际送达的每个按键与鼠标事件，点一下就能看出某个按钮到底有没有传到 lumux。
...
按下前缀键期间，状态栏右端会显示一个指示符，让这一模式状态可见。可修改其文本，或设为空以关闭：
