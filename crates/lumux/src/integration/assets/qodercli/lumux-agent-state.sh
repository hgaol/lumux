...
# Qoder supplies one JSON object on stdin (its hook schema mirrors Claude's).
...
try:
    subprocess.run(
        [os.environ["LUMUX_BIN"], "report-state", state, "--agent", "qodercli"],
        env=environment,
